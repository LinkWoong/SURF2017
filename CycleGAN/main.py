...
class CycleGAN():
    ...
    def model_setup(self):
        ...
        with tf.variable_scope("Model") as scope:
            ...
            self.rec_B = build_discriminator(self.input_B, "d_B")

            scope.reuse_variables()

            self.fake_rec_A = build_discriminator(self.fake_A, "d_A")
            self.fake_rec_B = build_discriminator(self.fake_B, "d_B")
            self.cyc_A = build_generator_resnet_9blocks(self.fake_B, "g_B")
            self.cyc_B = build_generator_resnet_9blocks(self.fake_A, "g_A")

            scope.reuse_variables()

            self.fake_pool_rec_A = build_discriminator(self.fake_pool_A, "d_A")
            self.fake_pool_rec_B = build_discriminator(self.fake_pool_B, "d_B")
